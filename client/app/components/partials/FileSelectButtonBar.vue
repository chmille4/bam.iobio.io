...
.inverted-file-button:active,
.inverted-file-button:focus {
  outline: none;
  border: 0;
}
.file label {
...
<template>
  <div>
    ...
    >
      <input
        id="url-input"
        placeholder="BAM/CRAM URL"
        v-model="selectedBamURL"
      />
      <input
        id="bai-url-input"
        placeholder="BAI/CRAI URL (optional)"
        v-model="selectedBaiURL"
      />
      <button id="bam-url-go-button" @click="openBamURL">Go</button>
    </div>
    <div style="text-align: center">
      <button class="inverted-file-button" @click="launchDemoData">
  ...
  methods: {
    ...
    openBamURL: function () {
      if (!validURL(this.selectedBamURL)) {
        alert(
          "Please enter a valid bam/cram url, including http:// or https:// in front",
        );
        return;
      }
