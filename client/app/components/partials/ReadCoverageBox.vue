...
<template>
	<div id="depth-distribution" class="panel">
		<div class="read-coverage-title">
			...
			>
				<div slot="body">
					The read coverage shows how the read coverage varies across the entire genome. The coloured numbers beneath
					represent chromosomes in the reference genome used and can be selected to view the read coverage in an
					individual chromosome. Selecting a different chromosome will cause all other metrics in bam.iobio to be
					recalculated based on reads sampled from that chromosome only. Once a chromosome is selected, you can also
					focus on a smaller region by dragging over the region of interest; again, all other metrics will then be
					recalculated for that region only.
				</div>
			</help-button>
			Read Coverage
		</div>
		...
		<div id="readDepthLoadingMsg" style="font-size: 50px; margin-top: 30px; color: #2687be">
			Initializing data
		...
		<div v-if="tooManyRefs" class="warning">
			Too many references to display. Use the dropdown to the left to select the reference
		</div>
