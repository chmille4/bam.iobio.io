	...
	<div class="multi-minibar-chart" ref="container">
		<svg class="multi-minibar-chart__svg">
			<g class="content" :transform="translateStr(yAxisWidth, 0)">
				...
				<g class="multi-minibar-chart_segments" :transform="translateStr(0, 0)">
					<g v-for="(points, i) in shownPoints" :key="ids[i]" :transform="segmentTransform(i)">
						<MinibarSegment
							color="SteelBlue"
							:width="widths[i]"
				...
				<g v-if="shownPoints.length > 0" class="multi-minibar-chart__buttons" :transform="translateStr(0, 0)">
					<g v-for="(id, i) in shownIds" :key="shownIds[i]" :transform="buttonTransform(i)">
						<rect
							class="multi-minibar-chart__button"
							:width="widths[i]"
							:height="buttonHeight"
							:style="rectStyle(shownIds[i])"
							@click="buttonClick(shownIds[i])"
						/>
						<text
							class="multi-minibar-chart__button__text"
							:x="widths[i] / 2"
							y="12"
							fill="#eee"
							text-anchor="middle"
...
export default {
	...
	watch: {
		...
		yAxisRange: function () {
			const scale = d3.scale.linear().domain([this.yAxisRange.min, this.yAxisRange.max]).range([this.segmentHeight, 0]);
