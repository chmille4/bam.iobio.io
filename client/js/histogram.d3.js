function histogramD3() {
	...
	function chart(selection, options) {
		...
		selection.each(function (data) {
			...
			if (!options.outliers) data = removeOutliers(data);
			...
			var avgLineG = gEnter
				...
				.attr('transform', function (d) {
					return 'translate(' + parseInt(x(d) + half) + ',' + 0 + ')';
				});

			avgLineG.append('line').attr('x1', 0).attr('x2', 0).attr('y1', innerHeight).attr('y2', -8);

			avgLineG.append('text').text('avg').attr('y', '-10');
			...
			var barEnter = bar
				...
				.attr('class', 'bar')
				.attr('transform', function (d) {
					return 'translate(' + x(d[0]) + ',' + innerHeight + ')';
				});
		...
		function removeOutliers(data) {
			...
			return data.filter(function (d) {
				return d[0] >= Math.max(q1 - iqr, 0) && d[0] <= q3 + iqr;
			});
		}
		...
		function quantile(arr, p) {
			...
			for (var i = 0; i < arr.length; i++) {
				currValue += arr[i][1];
				if (hMinus1Value == undefined && currValue >= h - 1) hMinus1Value = arr[i][0];
