				...
				var outlier = function () {
					...
					function quantile(arr, scale, p) {
						...
						for (var i = 0; i < arr.length; i++) {
							...
							if (hMinus1Value == undefined && currValue >= h - 1) hMinus1Value = scale(value(arr[i]));
			...
			function (require, module, exports) {
				var pileup = function () {
					...
					function layout(data) {
						...
						var values = data.map(function (d, i) {
							return [+startValue.call(layout, d, i), +endValue.call(layout, d, i)];
						});
						var xScale = d3.scale
							.linear()
			...
			function (require, module, exports) {
				var variant = function () {
					...
					function shape(d, i) {
						...
						else return diagonal(d) + diagonal({ x: xValue(d), y: yValue(d), w: -wValue(d) });
					}
				...
				var alignment = function () {
					...
					function chart(selection, opts) {
						...
						if (orientation == 'down') {
							...
							// update y axis
							if (yAxis) selection.select('.iobio-y.iobio-axis').transition().duration(0).call(yAxis);
						}
						...
						aln
							...
							.attr('transform', function (d, i) {
								var translate =
									...
									')';
								if (directionValue && directionValue(d, i) == 'reverse') return translate + ' rotate(180)';
							...
							.attr('points', function (d) {
								...
								if (directionValue)
									// draw arrow
									return '-0.1,' + -rH / 2 + ' 0,' + -rH / 2 + ' 0.1,0' + ' 0,' + rH / 2 + ' -0.1,' + rH / 2;
								// draw rectangle
								else return '-0.1,' + -rH / 2 + ' 0,' + -rH / 2 + ' 0,' + rH / 2 + ' -0.1,' + rH / 2;
							});
			...
			function (require, module, exports) {
				var bar = function () {
					...
					function chart(selection, opts) {
						...
						if (innerHeight < 0) {
							console.log(
								'Negative inner height ' + innerHeight + ' calculated for bar chart. Change height or margins.'
							);
							console.trace();
							return;
						}
		...
		15: [
			function (require, module, exports) {
				...
				var base = function () {
					...
					// Axes
					var xAxis = d3.svg.axis().scale(x).orient('bottom').tickFormat(utils.format_unit_names).ticks(5),
						yAxis = d3.svg
							.axis()
							.scale(y)
							.orient('left')
					...
					function chart(selection, opts) {
						...
						data = data.map(function (d, i) {
							return [xValue.call(data, d, i), yValue.call(data, d, i), wValue.call(data, d, i)];
						});
						...
						// Update the x-axis.
						if (xAxis) g.select('.iobio-x.iobio-axis').transition().duration(transitionDuration).call(xAxis);

						// Update the y-axis.
						if (yAxis) g.select('.iobio-y.iobio-axis').transition().duration(transitionDuration).call(yAxis);
						...
						if (tooltip) {
							...
							svg
								.on('mouseover', function (d, i) {
									...
									tt.transition().duration(transitionDuration).style('opacity', opacity);
				...
				var box = function () {
					...
					var sort = function (data, compression) {
						return data.sort(function (i, j) {
							var diffM =
								(quartilesValue(i)[1] - quartilesValue(j)[1]) / ((quartilesValue(i)[1] + quartilesValue(j)[1]) / 2);
							var diffQ1 =
								(quartilesValue(i)[0] - quartilesValue(j)[0]) / ((quartilesValue(i)[0] + quartilesValue(j)[0]) / 2);
							var diffQ3 =
								(quartilesValue(i)[2] - quartilesValue(j)[2]) / ((quartilesValue(i)[2] + quartilesValue(j)[2]) / 2);
						...
						data.forEach(function (d) {
							...
							if (medianClose && quartile1Close && quartile3Close && !d.uncompressable) {
								...
								quartilesValue(currBox)[1] =
									(quartilesValue(currBox)[1] * currBox.totalBoxes + quartilesValue(d)[1]) / (currBox.totalBoxes + 1);
								quartilesValue(currBox)[0] =
									(quartilesValue(currBox)[0] * currBox.totalBoxes + quartilesValue(d)[0]) / (currBox.totalBoxes + 1);
								quartilesValue(currBox)[2] =
									(quartilesValue(currBox)[2] * currBox.totalBoxes + quartilesValue(d)[2]) / (currBox.totalBoxes + 1);
					...
					function chart(selection, opts) {
						...
						//  		})
						var compressionNumber = box.selectAll('.compressionNumber').data(function (d) {
							return [d];
						});
			...
			function (require, module, exports) {
				...
				var gene = function () {
					...
					function chart(selection, opts) {
						...
						transcript.selectAll('.arrow').transition().duration(transitionDuration).attr('d', centerArrow);
				...
				var line = function (container) {
					...
					function chart(selection, opts) {
						selection.each(function () {
							...
							gEnter.attr('stroke-dasharray', totalLength + ' ' + totalLength).attr('stroke-dashoffset', totalLength);
		...
		21: [
			function (require, module, exports) {
				var multiLine = function () {
					...
					// Axes
					var xAxis = d3.svg.axis().orient('bottom').tickFormat(utils.format_unit_names).ticks(5);
					var yAxis = d3.svg
						.axis()
						.orient('left')
						.tickFormat(function (d) {
					...
					function chart(selection, opts) {
						...
						button
							...
							.style('display', function (d, i) {
								if (selected == 'all' || selected == nameValue(d, i)) return 'block';
								else return 'none';
							})
							.attr('transform', function (d, i) {
								return 'translate(' + x(d.globalPos) + ')';
			...
			function (require, module, exports) {
				var pie = function () {
					...
					function chart(selection, opts) {
						...
						// enter
						var pathEnter = path.enter().append('g').attr('id', id).attr('class', 'arc').style('fill', color);
						...
						if (transitionDuration != undefined && transitionDuration >= 0) {
							...
							path
								...
								.attr('transform', function (d) {
									...
									return 'translate(' + arcLabelPosition(d, 0.55) + ') rotate(' + angle + ')';
								})
								.text(function (d, i) {
									if (!nameValue) return;
...
*/
				var pieChooser = function () {
					...
					function chart(selection, opts) {
						...
						arc = d3.svg.arc().innerRadius(chart.innerRadius()).outerRadius(chart.radius());
						...
						arcs
							...
							.on('mouseout', function (d) {
								...
								if (clickedSlices.length == 0 && this != clickedSlice) {
									d3.select(this).select('path').transition().duration(150).attr('transform', 'translate(0,0)');
								}
					...
					chart._unclickSlice = function (clickedSlice) {
						...
						d3.select(clickedSlice).select('path').transition().duration(150).attr('transform', 'translate(0,0)');
					...
					chart._selectSlice = function (d, i, gNode, deselectPrevSlice) {
						...
						if (options.showTooltip) {
							...
							var matrix = theSlice
								.getScreenCTM()
								.translate(+theSlice.getAttribute('cx'), +theSlice.getAttribute('cy'));
							// position tooltip
							_tooltip()
								.html(name(d.data))
			...
			function (require, module, exports) {
				var scatter = function () {
					...
					function chart(selection, opts) {
						...
						if (innerHeight < 0) {
							console.log(
								'Negative inner height ' + innerHeight + ' calculated for scatter chart. Change height or margins.'
							);
							console.trace();
							return;
						}
