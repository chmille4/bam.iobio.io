function bamD3(container, heightPct, color) {
	...
	var svg = d3
		...
		.attr(
			'viewBox',
			'0 0 ' + parseInt(width + margin.left + margin.right) + ' ' + parseInt(height + margin.top + margin.bottom)
		)
	...
	function my(values, options) {
		var minMax = d3.extent(values, function (elem) {
			return elem.position;
		});
		var avgDepth = d3.mean(values, function (elem) {
			return elem.length;
		});
		var numBins = 20;
		...
		x.domain(minMax);
		if (x.domain()[0] == x.domain()[1]) x.domain([x.domain()[0] - 10, x.domain()[1] + 10]);
		...
		data.forEach(function (d) {
			d.y = d3.mean(d, function (elem) {
				return parseInt(elem.length * 100) / 100;
			});
		});
		...
		var lineFunction = d3.svg
			.line()
			.x(function (d) {
				return x(d.x) + x(x.domain()[0] + data[0].dx) / 2;
			})
			.y(function (d) {
				return y(d.y);
			})
		...
		if (svg.select('path').empty()) {
			...
			svg.select('path').transition().duration(duration).attr('d', lineFunction(data));
		}
		...
		if (svg.select('.y.axis').empty()) {
			svg.append('g').attr('class', 'y axis').style('font-size', '9px').call(yAxis);
		} else {
